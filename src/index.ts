...
async function openSentinelPullRequest(
  ...
  const title = `🛰️ SentinelCI: Dockerfile fixes (${agents || "AI"})`;
...
function buildPullRequestBody(
  ...
  const lines: string[] = [];
  lines.push("## 🛰️ SentinelCI — Automatic Dockerfile fix");
  lines.push("");
  lines.push(
    `This PR applies the fix suggested by the **SentinelCI API** to the \`${filePath}\` file (base: \`${baseBranch}\`).`
  );
  lines.push("");
  lines.push(`**Status:** ${response.status}`);
  lines.push(`**Agents run:** ${response.selected_agents.join(", ") || "—"}`);
  lines.push("");
  lines.push("### 📝 Agents comment");
  lines.push("");
  lines.push(response.comment || "_No comment returned by the API._");
  lines.push("");

  const metrics = response.metrics;
  if (metrics) {
    lines.push("### 📊 Metrics (token usage & cost)");
    lines.push("");
    lines.push("| Stage | Agent | Model | Input | Output | Total | Cost | Duration |");
    lines.push("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |");
  ...
  lines.push("---");
  lines.push("_Automatically generated by the **SentinelCI** GitHub Action._");
